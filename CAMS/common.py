...
class Evaluator:
    ...
    def __init__(self, dset):
        ...
        self.pairs = torch.LongTensor(pairs).to('cuda:0')
        ...
        self.oracle_obj_mask = torch.stack(oracle_obj_mask, 0).to('cuda:0')
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk=1):
        ...
        def _process(_scores):
            ...
            attr_match = (attr_truth.unsqueeze(1).repeat(1, topk) == _scores[0][:, :topk].to('cpu'))
            obj_match = (obj_truth.unsqueeze(1).repeat(1, topk) == _scores[1][:, :topk].to('cpu'))
        ...
        obj_oracle_match = (
                attr_truth == predictions['object_oracle'][0][:, 0].to('cpu')).float()  # object is already conditioned
        obj_oracle_match_unbiased = (attr_truth == predictions['object_oracle_unbiased'][0][:, 0].to('cpu')).float()
